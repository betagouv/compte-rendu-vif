import { assign, createActor, setup, StateValueFrom } from "xstate";

export const menuMachine = setup({
  types: {
    context: {} as { clause: { label: string; clauseId: string } | null },
    events: {} as
      | { type: "TOGGLE" }
      | { type: "OPEN" }
      | { type: "CLOSE" }
      | { type: "GO_TO_HELP" }
      | { type: "GO_TO_CLAUSES"; clause: { label: string; clauseId: string } }
      | { type: "BACK" },
  },
}).createMachine({
